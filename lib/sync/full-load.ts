...
async function getTableInfo(sourceConfig: DbConnectionConfig, tableName: string): Promise<TableInfo> {
  console.log(`Simulating getTableInfo for ${tableName} at ${sourceConfig.host || sourceConfig.filePath || sourceConfig.url}`);
...
async function query(sourceConfig: DbConnectionConfig, sql: string, params?: any[]): Promise<any[]> {
  console.log(`Simulating query "${sql}" with params ${JSON.stringify(params)} at ${sourceConfig.host || sourceConfig.filePath || sourceConfig.url}`);
  // Simulate based on sourceConfig.type if needed
  ...
  const recordsPerPage = params && params.length > 1 ? (params[1] - params[0]) : 10; // Approximate from limit/offset or chunkSize

  return Array(recordsPerPage).fill(0).map((_, i) => {
    const record: any = {};
...
async function insertBatch(targetConfig: DbConnectionConfig, tableName: string, data: any[]): Promise<void> {
    if (data.length === 0) {
        console.log(`Skipping insertBatch for ${tableName} at ${targetConfig.host || targetConfig.filePath || targetConfig.url} as data is empty.`);
        return;
    }
    console.log(`Simulating insertBatch of ${data.length} records into ${tableName} at ${targetConfig.host || targetConfig.filePath || targetConfig.url}`);
...
class CheckpointManager {
  ...
  async saveCheckpoint(position: CheckpointData): Promise<void> {
    console.log(`Saving checkpoint for ${this.jobId} at position: ${JSON.stringify(position)} to ${this.checkpointFile}`);
  ...
  async getLastCheckpoint(): Promise<CheckpointData | null> {
    console.log(`Retrieving last checkpoint for ${this.jobId} from ${this.checkpointFile}`);
    try {
      ...
      console.error(`Failed to read or parse checkpoint for ${this.jobId}:`, err);
...
async function parallelTableExtraction(
...
): Promise<{ table: string; totalRecords: number; processedRecords: number; status: string }> {
  console.log(`Starting parallel extraction for table: ${tableName} from source: ${sourceConfig.type} to target: ${targetConfig.type}`);
  ...
  if (tableInfo.estimatedRows === 0) {
    console.log(`Table ${tableName} is empty or estimated rows is 0. Skipping extraction.`);
    // Though executeFullLoad might have already set progress, this ensures a specific message for empty tables.
    updateJobStatus(jobId, { message: `Table ${tableName} is empty, no records to extract.`, progress: 100 });
    return {
        table: tableName,
        totalRecords: 0,
        processedRecords: 0,
  ...
  const checkpointManager = new CheckpointManager(`full_load_${jobId}_${tableName}`);
  ...
  if (lastCheckpoint && typeof lastCheckpoint.lastId === 'number') {
    ...
    startPosition = lastCheckpoint.lastId; // Resume after the last successfully processed ID
     ...
     console.warn("String primary key checkpoint resume not fully implemented in simulation. Starting from beginning.");
  }


  const processChunk = async (chunkStartId: number | string, currentChunkSize: number) => {
    // For numeric PKs. String PKs or no PKs would need a different strategy (e.g. offset-based)
    if (typeof chunkStartId !== 'number') {
        console.error("This simulated chunk processing requires a numeric start ID.");
        return 0;
    }
    const chunkEndId = chunkStartId + currentChunkSize;
    console.log(`Processing chunk for PK range from ${chunkStartId} to < ${chunkEndId}`);
    ...
    await checkpointManager.saveCheckpoint({ lastId: chunkEndId, timestamp: new Date().toISOString() });
    return data.length;
  };
  ...
  for (let currentId = startPosition; currentId < tableInfo.estimatedRows; ) {
    ...
    if (totalChunks > 0) {
      const progressPercentage = Math.min(99, Math.round((chunksProcessed / totalChunks) * 100)); // Cap at 99 until truly done
      updateJobStatus(jobId, {
       ...
       updateJobStatus(jobId, { status: 'running', message: `Processed ${totalProcessed} records for table ${tableName} (chunk progress unavailable).`});
    }

    if (processedInChunk < currentChunkSize && (currentId + processedInChunk < tableInfo.estimatedRows)) {
        ...
        console.warn(`Chunk for PK range ${currentId} to ${currentId + currentChunkSize} returned ${processedInChunk} records. Expected up to ${currentChunkSize}.`);
  ...
  console.log(`Processed ${totalProcessed} records for table ${tableName} in this run.`);
  console.log(`Overall progress: ${overallProcessedSinceStart} / ${tableInfo.estimatedRows} records for table ${tableName}.`);
  ...
  return {
    ...
    status: overallProcessedSinceStart >= tableInfo.estimatedRows ? 'complete' : 'partial'
  };
}
...
export async function executeFullLoad(
  ...
  initJob(jobId);
  updateJobStatus(jobId, { status: 'running', message: `Initializing full load for table ${tableName}...`, progress: 0 });
  ...
  try {
    ...
    updateJobStatus(jobId, { status: 'running', message: `Starting extraction for table ${tableName}...`, progress: 5 });
    ...
    const result = await parallelTableExtraction(jobId, sourceConfig, targetConfig, tableName, options);
    ...
    console.log(`Full load result for ${tableName}: ${JSON.stringify(result, null, 2)}`);
    updateJobStatus(jobId, {
      status: 'completed',
    ...
    console.error(`Error during full load for job ${jobId}, table ${tableName}:`, errorMessage);
    updateJobStatus(jobId, { status: 'failed', message: `Full load failed: ${errorMessage}`, progress: 0 }); // Assuming progress is 0 or last known if error occurs
